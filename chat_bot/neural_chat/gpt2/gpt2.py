...
class GPT2(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.tokenizer = PreTrainedTokenizerFast.from_pretrained(gpt2_type_or_checkpoint)
        self.tokenizer.pad_token = "<pad>"  # newline token #??? 왜 띄어쓰기를?
    ...
    def _encode_dialog(self, context, dialog):
        return self._encode(format_dialog(context, dialog) + "<sep>판매자:")[
            "input_ids"
        ]
    ...
    def generate_best_cut(
        ...
        **kwargs,
        ...
        return self.generate_best(context, dialog[-2:], **kwargs) # 도대체 뭘 cut한다는건지?
